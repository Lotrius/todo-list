...
class Item extends Component {
  ...
  render() {
    ...
    return (
      ...
      <div
        className={
          'mw6 center pa2 mt3 br2-ns ba b--black-10 overflow-auto ' +
          (todo.completed ? 'bg-light-green' : 'bg-light-blue')
        }
      >
        <div className='dib fl'>{this.printListItem(todo, ind)}</div>
  ...
  printListItem = (item, index) => {
    ...
    return this.state.editing ? (
      ...
      <label
        style={taskStyle}
        onClick={this.props.toggleComplete.bind(this, index)}
      >
        {item.task}
      </label>
    );
  ...
  removeOrCancelButton = index => {
    return this.state.editing ? (
      ...
      <button type='button' onClick={() => this.props.removeItem(index)}>
        X
      </button>
    );
  };
  ...
  editOrSaveButton = index => {
    ...
    return this.state.editing ? (
      ...
      <div className='pr2 dib'>
        <button type='button' onClick={this.saveEdit.bind(this, index)}>
          Save
        </button>
      </div>
    ) : (
      ...
      <div className='pr2 dib'>
        <button type='button' onClick={this.changeEditValue}>
          Edit
        </button>
      </div>
    );
